...
class CustomObstacleTowerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        brain_state = self._env.step(action)[self.brain_name]
        n_agents = len(brain_state.agents)
        self._check_agents(n_agents)
        self._current_state = brain_state

        obs, reward, done, info = self._single_step(brain_state)
        self.game_over = done
    ...
    def _custom_reward(self, state):
        ...
        if self._prev_key_obs is not None:
            prev_key_num = np.argmax(self._prev_key_obs, axis=0)
            curr_key_num = np.argmax(key_obs, axis=0)
            #prev_num_keys = np.count_nonzero(self._prev_key_obs)
            #curr_num_keys = np.count_nonzero(key_obs)
            if curr_key_num > prev_key_num:
                custom_reward += (curr_key_num - prev_key_num) * 0.5
        self._prev_key_obs = key_obs.copy()
