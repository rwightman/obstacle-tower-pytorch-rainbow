...
class SegmentTree:
    def __init__(self, size):
        ...
        self.sum_tree = np.zeros(
            (2 * size - 1,), dtype=np.float32)  # Initialise fixed size tree with all (priority) zeros
...
class ReplayMemory:
    def __init__(self, args, capacity, obs_space):
        ...
        self.blank_trans = Transition(0, torch.zeros(obs_space.shape, dtype=torch.uint8), None, 0, False)

    # Adds state and action at time t, reward and terminal at time t + 1
    def append(self, state, action, reward, terminal):
        # FIXME Only store last frame and discretise to save memory
        if state.dim() > len(self.obs_space.shape):
            print(state.dim(), self.obs_space.shape)
            state = state[-1]
        state = (127.5 * state + 127.5).to(dtype=torch.uint8, device=torch.device('cpu'))
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        ret = torch.tensor(
            [sum(self.discount ** n * transition[self.history + n - 1].reward for n in range(self.n))],
            dtype=torch.float32,
            device=self.device)
        ...
        return prob, idx, tree_idx, state, action, ret, next_state, nonterminal
    ...
    def __next__(self):
        ...
        if self.history == 1:
            ...
            state = torch.stack(state_stack, 0)  # Agent will turn into batch
